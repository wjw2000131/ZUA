...
def make_net(in_channels, conf, include_last_relu=True):
    ...
    def make_layer(layer_cfg):
        ...
        # inplace=True ： 节省反复申请与释放内存的空间与时间
    ...
    def forward(self, convouts:List[torch.Tensor]):
        ...
        for lat_layer in self.lat_layers:
            ...
            if j < len(convouts) - 1:
                ...
                # 对x进行上采样，self.interpolation_mode = bilinear
                x = F.interpolate(x, size=(h, w), mode=self.interpolation_mode, align_corners=False)

            # x每次加上 卷积（lat_layer）第j个选择特征层层输入数据
...
class Yolact(nn.Module):
    ...
    def __init__(self):
        ...
        if cfg.mask_type == mask_type.direct:
            ...
            # cfg.mask_proto_net : [(256, 3, {'padding': 1})] * 3 + [(None, -2, {}), (256, 3, {'padding': 1})] + [(32, 1, {})]
            # 图像维度放大网络块，可能包括逆卷积和上采样 返回 Protonet的数量为 cfg.mask_dim
            self.proto_net, cfg.mask_dim = make_net(in_channels, cfg.mask_proto_net, include_last_relu=False)
            if cfg.mask_proto_bias:
                cfg.mask_dim += 1

        # cfg.backbone.selected_layers ：[3] + list(range(5, 10))(vgg16)
        ...
        if cfg.fpn is not None:
            ...
            # 选择backbone.channel中对应的selected_layers层数据以列表的形式输入到FPN中进行初始化
